...
indice_seleccionado = st.selectbox(":orange[Selecciona ïndice/Portolio a analizar:]", [ "Dow Jones","NASDAQ 100", "Portfolio"],
        ...
        with st.spinner("Validando símbolos de forma eficiente..."):
            try:
                ...
                if isinstance(df_val.columns, pd.MultiIndex):
                    ...
                    if df_val.empty:
                        st.error("No se pudo validar ningún símbolo. Intenta con otros.")
                        st.stop()
                    else:
                        tickers = posibles
...
if ticker_sel and ticker_sel in precios.columns:
    ...
    if isinstance(precios[ticker_sel], pd.Series):
        ...
        st.error(f"No se pudo obtener datos válidos para {ticker_sel}.", icon=":material/error:")
        st.stop()
    ...
    with st.container(border=True):
        ...
        if fecha_cruce:
            ...
            st.subheader(":orange[### No se encontró cruce dorado reciente.]")
